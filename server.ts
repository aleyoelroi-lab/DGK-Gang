...
// Shared Weird Missions List for Server Authority
const SERVER_MISSIONS = [
  {
    ...
    targetPosition: { x: -24, y: 1.0, z: 36 },
    targetCount: 3,
    currentCount: 0,
...
function broadcastToRoom(room: RoomState, msg: any, excludeClientId?: string) {
...
wss.on('connection', (ws: WebSocket, req) => {
  ...
  // Send Initial Join Packet to the connecting client
  ...
  // Broadcast to all other players that a new cat operative joined
  broadcastToRoom(
    room,
    {
      type: 'player_joined',
  ...
  ws.on('message', (raw: string) => {
    try {
      ...
      if (data.type === 'update_profile') {
        ...
        // Any 1 player progressing or completing the shared mission
        if (room.sharedMission && !room.sharedMission.completed) {
          ...
          if (room.sharedMission.currentCount >= room.sharedMission.targetCount) {
            ...
            room.isKissHuntActive = false; // SAVES ALL CATS FROM KISS!
            broadcastToRoom(room, {
        ...
        // WebRTC peer-to-peer audio forwarding for voice chat
...
// Server Tick Loop (1 second ticker for Shared Mission Countdown & Kiss Hunt Mode)
setInterval(() => {
  rooms.forEach((room) => {
    ...
    if (room.sharedMission && !room.sharedMission.completed) {
      ...
      // 30 seconds warning before times up notification!
      ...
      // If quest duration runs out before completion -> TRIGGER KISS HUNT!
      if (room.sharedMission.secondsLeft <= 0 && !room.isKissHuntActive) {
        room.isKissHuntActive = true;
        room.kissHuntTimer = 60; // 60s of thrilling chase
        broadcastToRoom(room, {
          type: 'kiss_hunt_started',
          message: '🚨 TIME IS UP! The scary human is coming to kiss all uncompleted cats! CLIMB HIGH TO ESCAPE!',
        });
      }
      ...
      if (room.isKissHuntActive) {
        ...
        if (room.kissHuntTimer <= 0) {
          // Kiss hunt ends -> start next fresh quest
          room.isKissHuntActive = false;
          room.sharedMission = pickRandomMission();
          broadcastToRoom(room, {
            type: 'new_mission_started',
